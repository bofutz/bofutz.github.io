  ...
  setup() {
    ...
    const openModal = (item = null) => {
      if (item) {
        ...
        Object.assign(form, { id: null, etf_code: "", etf_name: "", sort_order: 0, enabled: true });
      }
    ...
    <div class="space-y-4 select-none">
      <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
        <div>
          ...
          <p class="text-xs text-slate-400 mt-1">管理前台通用的公开监控标的清单</p>
        </div>
        <div class="flex gap-2">
          <button @click="batchImportVisible = true" class="text-xs bg-slate-100 text-slate-700 px-3 py-2 rounded-lg font-bold hover:bg-slate-200">批量导入</button>
          <button @click="openModal()" class="theme-bg text-white text-sm px-3.5 py-2 rounded-lg font-bold hover:opacity-90">添加标的</button>
          <button @click="loadShared" class="bg-white border px-3 py-2 rounded-lg text-sm hover:bg-slate-50"><i class="fa-solid fa-rotate-right" :class="{'animate-spin': loading}"></i></button>
        </div>
      </div>
      ...
      <div class="bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm">
        ...
        <div v-else-if="!sharedList.length" class="text-center py-12 text-slate-400 text-sm">暂无通用标的</div>
          ...
          <table class="w-full text-sm text-left whitespace-nowrap">
            ...
            <tbody class="divide-y divide-slate-50">
              <tr v-for="w in sharedList" :key="w.id" class="hover:bg-slate-50">
                ...
                <td class="py-3 px-4"><span :class="w.enabled ? 'text-emerald-500 font-bold' : 'text-slate-400'">{{ w.enabled ? '启用' : '停用' }}</span></td>
      ...
      <div v-if="modalVisible" class="fixed inset-0 modal-overlay z-[100] flex items-center justify-center p-4" @click.self="modalVisible = false">
        <div class="bg-white rounded-2xl w-full max-w-md p-6 space-y-3 shadow-2xl">
          ...
          <label class="flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" v-model="form.enabled"> 启用</label>
      ...
      <div v-if="batchImportVisible" class="fixed inset-0 modal-overlay z-[100] flex items-center justify-center p-4" @click.self="batchImportVisible = false">
        <div class="bg-white rounded-2xl w-full max-w-md p-6 space-y-3 shadow-2xl">
          ...
          <p class="text-xs text-slate-400">格式每行：代码,名称（如：510300,沪深300ETF）</p>
          <textarea v-model="batchText" rows="8" class="w-full border px-3 py-2 rounded-lg text-sm font-mono focus:theme-border outline-none"></textarea>
          <div class="flex justify-end gap-2 pt-2">
