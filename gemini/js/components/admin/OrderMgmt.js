...
export default {
  ...
  setup() {
    ...
    const loadOrders = async () => {
      loading.value = true;
      try {
        const res = await adminApi.fetchOrders(orderStatusFilter.value === "all" ? "" : orderStatusFilter.value);
    ...
    const approve = async (order) => {
      if (!confirm(`确认通过该订单？\n用户: ${order.username || order.register_username || '游客'}\n类型: ${order.order_type}`)) return;
      try {
    ...
    <div class="space-y-4 select-none">
      ...
      <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
        ...
        <div class="flex gap-2 flex-wrap items-center">
          ...
          <input v-model="searchQuery" type="text" placeholder="账号/单号" class="px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white outline-none focus:theme-border">
          <button @click="loadOrders" class="bg-white border px-3 py-2 rounded-lg text-sm hover:bg-slate-50"><i class="fa-solid fa-rotate-right" :class="{'animate-spin': loading}"></i></button>
        </div>
      </div>
      ...
      <div class="bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm">
        ...
        <div v-else-if="!filteredOrders.length" class="text-center py-12 text-slate-400 text-sm">
          暂无订单记录
        </div>
          ...
          <table class="w-full text-sm whitespace-nowrap text-left">
            ...
            <tbody class="divide-y divide-slate-50">
              <tr v-for="o in filteredOrders" :key="o.id" class="hover:bg-slate-50">
                ...
                <td class="py-3.5 px-4 font-bold text-slate-800">{{ o.username || o.register_username || '游客' }}</td>
                <td class="py-3.5 px-4">
                  <span class="bg-slate-100 px-2 py-0.5 rounded text-xs text-slate-600 font-medium">{{ o.plan_id }}</span>
                  <span class="text-[10px] ml-1.5 px-1.5 py-0.5 rounded"
                ...
                <td class="py-3.5 px-4 text-xs font-bold text-emerald-600">
                  {{ o.status === 'approved' ? (o.vip_days_granted ? ('+' + o.vip_days_granted + '天') : '定制激活') : '-' }}
                </td>
                <td class="py-3.5 px-4">
                ...
                <td class="py-3.5 px-4 text-right space-x-1">
                  <template v-if="o.status === 'pending'">
                    <button @click="approve(o)" class="theme-bg text-white text-xs px-3 py-1 rounded-lg font-bold hover:opacity-90">通过</button>
                    <button @click="reject(o)" class="bg-slate-100 text-slate-600 text-xs px-3 py-1 rounded-lg font-bold hover:bg-slate-200">驳回</button>
                  </template>
