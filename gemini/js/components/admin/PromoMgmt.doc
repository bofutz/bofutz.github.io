    ...
    <div class="space-y-4 select-none">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-xs text-slate-400 mt-1">设置全网优惠码、折扣力度、有效期与使用限制</p>
        </div>
        <div class="flex gap-2">
          <button @click="openModal()" class="theme-bg text-white text-sm px-3.5 py-2 rounded-lg font-bold hover:opacity-90">新增优惠码</button>
          <button @click="loadPromos" class="bg-white border px-3 py-2 rounded-lg text-sm hover:bg-slate-50"><i class="fa-solid fa-rotate-right" :class="{'animate-spin': loading}"></i></button>
        </div>
      </div>
      ...
      <div class="bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm">
        ...
        <div v-else-if="!promos.length" class="text-center py-12 text-slate-400 text-sm">暂无优惠码数据</div>
          ...
          <table class="w-full text-sm text-left whitespace-nowrap">
            ...
            <tbody class="divide-y divide-slate-50">
              <tr v-for="p in promos" :key="p.id" class="hover:bg-slate-50">
                ...
                <td class="py-3 px-4 text-xs font-mono text-slate-500">{{ formatDate(p.start_at) }} ~ {{ formatDate(p.end_at) }}</td>
                <td class="py-3 px-4"><span :class="p.enabled ? 'text-emerald-500 font-bold' : 'text-slate-400'">{{ p.enabled ? '启用' : '停用' }}</span></td>
      ...
      <div v-if="modalVisible" class="fixed inset-0 modal-overlay z-[100] flex items-center justify-center p-4" @click.self="modalVisible = false">
        <div class="bg-white rounded-2xl w-full max-w-md p-6 space-y-3 shadow-2xl">
          ...
          <input v-model="promoForm.code" placeholder="代码 (例如: DISCOUNT10)" class="w-full border px-3 py-2 rounded-lg text-sm font-mono uppercase focus:theme-border outline-none">
          <input v-model="promoForm.name" placeholder="名称 (例如: 新人优惠)" class="w-full border px-3 py-2 rounded-lg text-sm focus:theme-border outline-none">
          ...
          <div class="grid grid-cols-2 gap-2">
            ...
            <input type="number" v-model.number="promoForm.discount_value" placeholder="折扣值" class="border px-3 py-2 rounded-lg text-sm focus:theme-border outline-none">
          </div>

          <div class="grid grid-cols-2 gap-2">
            <div>
              <label class="text-[10px] text-slate-400 block">生效开始</label>
            ...
            <div>
              <label class="text-[10px] text-slate-400 block">生效截止</label>
          ...
          <label class="flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" v-model="promoForm.enabled"> 启用状态</label>
