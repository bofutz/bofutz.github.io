    ...
    <div class="space-y-4 select-none max-w-5xl">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-xs text-slate-400 mt-1">控制全网赠送规则、默认收款码、优惠码显隐与权限上限</p>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="bg-white rounded-xl border border-slate-100 p-5 space-y-3.5 shadow-sm">
          ...
          <div class="pt-2 border-t border-slate-50">
            ...
            <select v-model="settingsForm.promo_enabled" class="w-full border px-3 py-2 rounded-lg text-sm focus:theme-border outline-none font-medium">
              <option value="1">开启 (购买套餐界面展示优惠码输入框)</option>
              <option value="0">关闭 (购买套餐界面完全隐藏优惠码卡片)</option>
            </select>
          </div>
        </div>
        ...
        <!-- 收款码与默认通道配置 -->
        <div class="bg-white rounded-xl border border-slate-100 p-5 space-y-3.5 shadow-sm">
          <div class="text-sm font-bold text-slate-700 border-b pb-2">精准支付与默认收款码</div>
          <div>
            <label class="text-xs text-slate-500 mb-1 block">默认支付通道</label>
            <select v-model="settingsForm.default_pay_channel" class="w-full border px-3 py-2 rounded-lg text-sm focus:theme-border outline-none font-medium">
        ...
        <div class="bg-white rounded-xl border border-slate-100 p-5 space-y-3.5 shadow-sm lg:col-span-2">
          <div class="text-sm font-bold text-slate-700 border-b pb-2">前台 Footer 社交链接</div>
