...
createApp({
  setup() {
    ...
    const sendEmailCode = async () => {
      ...
      if (!authForm.username || !emailRegex.test(authForm.username)) { alert("请输入正确邮箱"); return; }
    ...
    const submitAuth = async () => {
      ...
      if (authMode.value === "register") {
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(authForm.username)) { alert("请填写合法邮箱"); return; }
    ...
    const getPastWeeks = (etf_code) => {
      ...
      pastData.forEach((item) => {
        ...
        if (!weekMap[monday]) {
          weekMap[monday] = { monday, days: [null, null, null, null, null], week_status: null };
        }
    ...
    const customDraftPlanId = ref("month");
    ...
    const openCustomEditor = () => {
      ...
      customDraftPlanId.value = vipPlans.value[0]?.id || "month";
      customEditorVisible.value = true;
    };

    const confirmCustomAndPay = async () => {
      ...
      if (!customDraftPlanId.value) { alert("请选择套餐"); return; }
      try {
        // 创建 pending 定制 + 跳转购买（订单在支付提交时带 custom）
        sessionStorage.setItem("pending_custom_items", JSON.stringify(items));
        sessionStorage.setItem("pending_custom_plan", customDraftPlanId.value);
        const plan = vipPlans.value.find((p) => p.id === customDraftPlanId.value);
        if (plan) selectTopUpPlan(plan);
    ...
    const openChart = (etfCode, type, index) => {
      ...
      if (isTopThree || isInFreeList) { showViewer(etfCode, type); return; }
      if (confirm("此为 VIP 专属图表。\n\n排名前三免费开放，新注册可获体验权限。")) {
    ...
    const selectTopUpPlan = (plan) => {
      ...
      topUpForm.originalAmount = plan.price;
    ...
    const applyPromo = async () => {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/promo/check`, {
          ...
          body: JSON.stringify({ plan_id: topUpForm.planId, promo_code: promoInput.value.trim() }),
        });
        const data = await res.json();
        if (data.success) {
    ...
    const fetchPlans = async () => {
      try {
        ...
        if (data.success && data.data?.length) {
          ...
          if (!vipPlans.value.find((p) => p.id === topUpForm.planId)) {
            selectTopUpPlan(vipPlans.value[0]);
          }
        }
      } catch (_) {}
    };
    ...
    const checkPaymentStatus = async () => {
      orderMessage.value = "正在查询...";
      if (isLoggedIn.value) {
        await fetchOrders();
        if (orderList.value.some((o) => o.status === "approved" && Date.now() - new Date(o.created_at).getTime() < 300000)) {
          orderMessage.value = "自动核验成功！VIP 已激活";
          fetchData();
          fetchCustomWatchlist();
          return;
        }
      }
      setTimeout(() => { orderMessage.value = "若已付款，请稍后刷新或提交单号"; }, 1000);
    };
    ...
    const submitOrder = async () => {
      ...
      try {
        ...
        const pendingItems = sessionStorage.getItem("pending_custom_items");
        const pendingPlan = sessionStorage.getItem("pending_custom_plan");
        if (topUpForm.orderType === "custom_watchlist" && pendingItems) {
          customItems = JSON.parse(pendingItems);
          if (pendingPlan) topUpForm.planId = pendingPlan;
        }
        ...
        const body = {
          ...
          order_type: topUpForm.orderType || "vip",
        };
        if (customItems) body.custom_items = customItems;
        if (!isLoggedIn.value) {
          body.register_username = payRegister.username;
    ...
    const sendChatMessage = () => {
      if (chatModule.sendChatMessage) chatModule.sendChatMessage();
        ...
        chatMessages.value.push({ sender: "user", content: chatInput.value, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }), type: "text" });
        chatInput.value = "";
      }
    };
    ...
    const openSocialModal = (p) => { currentSocialPlatform.value = p; socialModalVisible.value = true; };
    ...
    watch(currentRoute, (newRoute) => {
      ...
      if (newRoute === "#/plan") {
        const pendingPlan = sessionStorage.getItem("pending_custom_plan");
        if (pendingPlan) {
          const plan = vipPlans.value.find((p) => p.id === pendingPlan);
          if (plan) selectTopUpPlan(plan);
          topUpForm.orderType = "custom_watchlist";
        }
      }
    ...
    watch(sortedData, (newVal) => {
      freeEtfCodes.value = newVal && newVal.length > 0 ? newVal.slice(0, 3).map((i) => i.etf_code) : [];
    }, { immediate: true });
    ...
    return {
      ...
      getDayTooltip, getWeekTooltip, getMobileDayDate, getMobileWeekDate, getWeekDays, freeEtfCodes,
      sharedWatchlist, customWatchlist, customLoading, customEditorVisible, customDraftItems, customDraftPlanId,
      openCustomEditor, confirmCustomAndPay, payCustomPending, removeCustomItem, formatDateShort,
