    ...
    setup() {
        ...
        const copyReferralCode = () => {
            ...
            navigator.clipboard.writeText(referralCode.value).then(() => {
                alert(`专属邀请码 [${referralCode.value}] 已成功复制到剪贴板！`);
            }).catch(() => {
                alert(`复制失败，您的邀请码为：${referralCode.value}`);
            });
        };
        ...
        const getMobileDayDate = (dateStr) => getDayTooltip(dateStr);
        const getMobileWeekDate = (mondayStr) => getWeekTooltip(mondayStr);
        ...
        const isLatestDailyColumn = (idx, item) => idx === latestDailyColIndex.value;
        ...
        // 获取并按 sort_order 动态排序套餐
        const vipPlans = ref([
            { id: 'week', name: '体验周卡', price: 6.80, days: 7, tag: '热销', sort_order: 1 },
            { id: 'month', name: '包月畅享', price: 18.80, days: 30, tag: '推荐', sort_order: 2 },
            { id: 'quarter', name: '季度特惠', price: 48.80, days: 90, tag: '特惠', sort_order: 3 },
            { id: 'Semiannual', name: '半年超值', price: 88.80, days: 180, tag: '超值', sort_order: 4 }
        ]);

        const sortedVipPlans = computed(() => {
            return [...vipPlans.value].sort((a, b) => (a.sort_order ?? 99) - (b.sort_order ?? 99));
        });
        ...
        const fetchPlans = async () => {
            try {
                ...
                if (data.success && data.data && data.data.length > 0) {
                    ...
                    if (topUpForm.planId && !data.data.some(p => p.id === topUpForm.planId)) {
                        selectTopUpPlan(data.data[0]);
                    }
                }
            } catch (e) {}
        };
        ...
        const openChart = (etfCode, type, index, isHistorical = false) => {
            ...
            if (confirm('此为 VIP 付费专属监控图表，请登录或注册开通。\n\n🎉 排名前三标的免费开放，新注册即可领 1 天 VIP 权限！')) {
        ...
        // 对齐 admin.html 订单状态语义
        const formatStatus = (s) => {
            return s === 'approved' ? '已通过' : s === 'pending' ? '待审核' : '已驳回';
        };

        // ================= 答疑工单交互 (映射 admin.html) =================
        const userTickets = ref([]);
        const ticketModalVisible = ref(false);
        const viewTicketModalVisible = ref(false);
        const ticketSubmitLoading = ref(false);
        ...
        const submitTicket = async () => {
            if (!ticketForm.subject.trim() || !ticketForm.message.trim()) {
                alert('请填写工单主题和问题详细描述！');
                return;
            }
            ticketSubmitLoading.value = true;
            try {
                ...
                alert('工单已成功提交！工作人员将在后台处理回复。');
                ticketModalVisible.value = false;
                fetchUserTickets();
            } catch (err) {
                alert(err.message || '工单提交失败');
            } finally {
                ticketSubmitLoading.value = false;
            }
        };

        const openViewTicketModal = (ticket) => {
            currentTicketDetail.value = ticket;
            viewTicketModalVisible.value = true;
        };
        ...
        const generateFloatingAmount = (basePrice) => {
            ...
            return ((Number(basePrice) || 0) + randCents).toFixed(2);
        };
        ...
        const checkPaymentStatus = async () => {
            orderMessage.value = '🔍 正在轮询 V 免签后台通知...';
            if (isLoggedIn.value) {
                await fetchOrders();
                if (
            ...
            setTimeout(() => {
                orderMessage.value =
                    '💡 若已付完款，系统监听一般在 3 秒内完成，您可稍后查看个人中心或刷新页面';
            }, 1200);
        };
        ...
        const submitOrder = async () => {
            ...
            try {
                ...
                orderMessage.value = '✅ 提交成功！请等待工作人员核验开通...';
                topUpForm.txId = '';
                setTimeout(() => {
                    fetchOrders();
                }, 1500);
