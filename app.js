 ...
 * 波幅探长 - 主业务逻辑脚本 (app.js)
 * 已修复：
 * 1. 日期解析函数严重错误（导致全站数据无法加载、Vue 无法正常工作）
 * 2. weekDays 数组被当字符串使用的多处逻辑错误
 * 3. 文件末尾残留的 EOF / 垃圾字符导致语法错误（Vue 无法 mount，模板原文显示）
 * 4. 社交弹窗相关变量已正确暴露
 * 5. 日线图标跟随最新有数据列、周线始终有图标、手机端证券风格、日期前缀等
 */
...
createApp({
    setup() {
        ...
        const formatMobileStatus = (status) => {
            ...
            if (!match) return '-';
            // 去掉符号，只留绝对值数字
            return Math.abs(parseFloat(match[0])).toString();
        };
        ...
        const formatMobileWeekLabel = (mondayStr) => {
            ...
            const weekNumStr = getWeekNumberInMonth(mondayStr); // 第x周
        ...
        const sortedData = computed(() => {
            ...
            // 再补全那些只有图表、当前周完全没数据的标的（保证所有有图的都能显示）
        ...
        // 社交 Modal 状态（解决截图中 {{ currentSocialPlatform }} 原文显示问题）
