...
createApp({
  setup() {
    ...
    const openChart = (etfCode, type, index) => {
      ...
      const isTop = typeof index === "number" && index >= 0 && index < freeTopN.value;
      const isInFreeList = freeEtfCodes.value.includes(etfCode);
      if (isTop || isInFreeList) { showViewer(etfCode, type); return; }
    ...
    watch(sortedData, (newVal) => {
      ...
      freeEtfCodes.value = newVal && newVal.length > 0 ? newVal.slice(0, n).map((i) => i.etf_code) : [];
    }, { immediate: true });
