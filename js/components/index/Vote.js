/**
 * 波幅探长 - 票选监控分块组件
 * 功能：全网会员票选最想监控的标的，每月初取当时 Top50 作为通用监控列表
...
export default {
  ...
  setup() {
    ...
    const userStatus = reactive({
      hasQualified: false,       // 是否具备投票资格（月付及以上）
      monthlyLimit: 10,          // 后台配置的每月上限
      votesUsed: 0,
      votesRemaining: 0,
      myVotes: [],               // 我本月已投的标的
    });
    ...
    const loadVoteData = async () => {
      ...
      try {
        ...
        // 最多展示前 200
        rankings.value = (rankRes.data || []).slice(0, 200);
    ...
    const openVoteModal = (presetCode = "", presetName = "") => {
      ...
      // 如果已有代码，自动触发一次名称识别
      if (presetCode && presetCode.length === 6) {
        onCodeInput();
      }
    };
    ...
    const isVotedByMe = (code) => {
      return userStatus.myVotes.some((v) => v.etf_code === code);
    };
    ...
    <div class="max-w-6xl mx-auto space-y-4 select-none">
      ...
      <div v-if="store.isLoggedIn && userStatus.myVotes.length > 0" class="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          >
            ...
            <button
              @click="cancelVote(v.id)"
              class="text-slate-400 hover:text-red-500 ml-1"
              title="撤销投票"
            >
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div v-else-if="!rankings.length" class="text-center py-14 text-slate-400">
          ...
          <button
            @click="openVoteModal()"
            class="mt-4 text-xs theme-bg text-white px-4 py-2 rounded-lg font-bold"
          >
            立即投票
          </button>
        </div>
          ...
          <table class="w-full text-sm text-center whitespace-nowrap">
            ...
            <tbody class="divide-y divide-slate-50">
              ...
              >
                ...
                <td class="py-3 px-3 text-left font-mono font-bold text-slate-800">
                  {{ item.etf_code }}
                </td>
                <td class="py-3 px-3 text-left font-medium text-slate-700 max-w-[140px] truncate">
                  {{ item.etf_name || item.etf_code }}
                </td>
                <td class="py-3 px-3 font-bold text-orange-500 font-mono">
                  {{ item.vote_count }} 票
                </td>
                <td class="py-3 px-3">
                  <div class="w-20 sm:w-24 bg-slate-100 rounded-full h-1.5 mx-auto overflow-hidden">
                    <div
                      class="theme-bg h-full rounded-full transition-all"
                ...
                <td class="py-3 px-3 text-right">
                  <button
                    v-if="isVotedByMe(item.etf_code)"
                    class="text-xs text-emerald-600 font-medium cursor-default"
                  >
                    <i class="fa-solid fa-check mr-0.5"></i>已投
                  </button>
                  <button
                    v-else
                    @click="openVoteModal(item.etf_code, item.etf_name)"
      ...
      >
        <div class="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl space-y-4 p-6">
          ...
          <div class="space-y-3">
            ...
            <div>
              <label class="text-xs font-bold text-slate-600 mb-1.5 block">
                标的名称
                <span class="font-normal text-slate-400">（自动识别，可修改）</span>
              </label>
              <div class="relative">
                <input
                  v-model="voteForm.etfName"
                  type="text"
                  placeholder="识别中或手动填写..."
          ...
          <div class="flex justify-end gap-2 pt-1">
            ...
            <button
              @click="submitVote"
              :disabled="submitLoading || searchingName || !voteForm.etfCode"
