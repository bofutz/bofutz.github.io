  ...
  setup() {
    ...
    const loadPlans = async () => {
      ...
      try {
        ...
        // 每用户限购：max_buy_times>0 且 user_bought_count 已达上限则隐藏（需后端返回这两字段）
        vipPlans.value = all.filter((p) => {
          if (p.enabled === 0) return false;
          if (p.plan_type === "custom" || p.plan_type === "chart") return false;
          const lim = Number(p.max_buy_times ?? p.per_user_limit ?? 0);
          if (lim > 0) {
            const bought = Number(p.user_bought_count ?? p.bought_count ?? 0);
            if (bought >= lim) return false;
          }
          return true;
        });
