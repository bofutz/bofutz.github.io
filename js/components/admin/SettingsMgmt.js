/* ========================= FILE: .\js\components\admin\SettingsMgmt.js ========================= */

/**
 * 波幅探长 - 后台系统设置（整合版）
 * 含：微信免密直登公众号配置、注册赠送、免费 TOP、收款码、
    ...
    <div class="space-y-5 max-w-4xl select-none">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p class="text-[11px] text-slate-400 mt-0.5">微信免密登录 → 优惠码 → 注册与邀请 → 自主查询 → 支付 → 票选 → 广告 → 社交</p>
        </div>
      ...
      <template v-else>
        ...
        <section class="bg-white rounded-xl border border-slate-100 p-5 space-y-3 shadow-sm">
          <div class="flex items-center justify-between border-b pb-2">
            <h3 class="font-bold text-slate-700 text-sm flex items-center gap-1.5">
              <i class="fa-brands fa-weixin text-emerald-500 text-base"></i> 微信公众号免密直登设置
            </h3>
            <span class="text-[10px] bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full font-bold">方案 A 免密转化</span>
          </div>
          <p class="text-[11px] text-slate-400 leading-relaxed">
            配置后，前端登录弹窗将直接展示该二维码。用户扫码关注公众号并回复验证码即可免密登录/静默注册，极大降低使用门槛。
          </p>
          <label class="text-xs space-y-1 block">
            <span class="text-slate-500">公众号关注二维码图片直链（支持 R2 / CDN 直链）</span>
            <input v-model="form.gzh_qr_url" type="text" placeholder="https://pub-xxx.r2.dev/gzh_qrcode.png" class="w-full border rounded-lg px-3 py-2 text-sm font-mono focus:theme-border outline-none">
          </label>
        </section>

        <!-- 优惠码（内嵌于系统设置首位） -->
        <section class="bg-white rounded-xl border border-slate-100 p-5 space-y-3 shadow-sm">
          <h3 class="font-bold text-slate-700 text-sm border-b pb-2">优惠码管理</h3>
          <PromoMgmt />
        </section>
        ...
        <section class="bg-white rounded-xl border border-slate-100 p-5 space-y-4 shadow-sm">
          ...
          <p class="text-[11px] text-slate-400 leading-relaxed">
            ...
            交易日 <code class="bg-slate-50 px-1 rounded">07:00 / 15:35 / 19:00 / 22:00</code> 拉半日+日线（15:35 含监控列表约 25 分钟，其余约 10 分钟）；
