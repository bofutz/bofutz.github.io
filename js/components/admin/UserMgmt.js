  ...
  setup() {
    ...
    const submitResetPwd = async () => {
      try {
        ...
        store.showToast(res.message || "密码已重置为 bofutz");
    ...
    <div class="space-y-4 select-none">
      <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
        <div>
          ...
          <p class="text-xs text-slate-400 mt-0.5">充天数 · 重置密码 · 删除均需谨慎操作</p>
        </div>
        <div class="flex gap-2 flex-wrap">
          <input
            v-model="searchQuery"
            type="text"
            placeholder="搜索账号 / 邀请码 / IP"
            class="px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white outline-none focus:theme-border sm:w-48"
          >
          <button
            @click="openBatchCharge"
            class="text-xs bg-emerald-50 text-emerald-600 border border-emerald-100 px-3 py-2 rounded-lg font-bold hover:bg-emerald-100"
          >
            批量充天数
          </button>
          <button @click="loadUsers" class="bg-white border px-3 py-2 rounded-lg text-sm hover:bg-slate-50">
      ...
      <div class="bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm">
        ...
        <div v-else-if="!filteredUsers.length" class="text-center py-14 text-slate-400 text-sm">
          暂无用户数据
        </div>
            ...
            <tbody class="divide-y divide-slate-50">
              <tr v-for="u in filteredUsers" :key="u.id" class="hover:bg-slate-50">
                ...
                <td class="py-3 px-4 font-bold text-slate-800">
                  {{ u.username }}
                  <div v-if="u.referred_by" class="text-[10px] text-slate-400 font-normal">
                    邀请人: {{ u.referred_by }}
                  </div>
                </td>
                <td class="py-3 px-4 font-mono text-xs text-slate-500">{{ u.ip || '-' }}</td>
                <td class="py-3 px-4 font-mono font-bold"
                    :class="(u.shared_vip_days || u.vip_days_left) > 0 ? 'text-emerald-600' : 'text-slate-400'">
      ...
      <div v-if="chargeModalVisible" class="fixed inset-0 modal-overlay z-[100] flex items-center justify-center p-4" @click.self="chargeModalVisible = false">
        <div class="bg-white rounded-2xl w-full max-w-sm p-6 space-y-3 shadow-2xl">
          ...
          <p class="text-xs text-slate-400">正数为增加，负数为扣减</p>
          <input type="number" v-model.number="chargeDays" class="w-full border px-3 py-2 rounded-lg text-sm focus:theme-border outline-none" placeholder="天数">
      ...
      <div v-if="resetPwdVisible" class="fixed inset-0 modal-overlay z-[100] flex items-center justify-center p-4" @click.self="resetPwdVisible = false">
        <div class="bg-white rounded-2xl w-full max-w-sm p-6 space-y-3 shadow-2xl">
          ...
          <p class="text-xs text-slate-400">请输入管理密钥确认，密码将重置为 <code class="bg-slate-100 px-1 rounded">bofutz</code></p>
