...
import { formatDateExact, formatDateShort } from "../../utils.js";
import {
  isLoggedIn, isVip, username, vipDaysLeft, referralCode, clearLoginState,
} from "../../auth.js";
  ...
  setup(props) {
    ...
    const fetchPlans = async () => {
      try {
        ...
        if (data.success) vipPlans.value = data.data || [];
      } catch (_) {}
    };
    ...
    const formatStatus = (s) =>
      s === "approved" ? "已完成" : s === "pending" ? "审核中" : "已取消";
    ...
    return {
      ...
      openCustomEditor: props.openCustomEditor,
      orderList,
      customWatchlist,
      customLoading,
      inviteeList,
      ...
      <template v-else>
          ...
          <div class="flex items-center justify-between gap-3">
            <div>
              ...
              <div class="flex items-center gap-2 flex-wrap">
                <span class="text-xl sm:text-2xl font-bold" :class="isVip ? 'theme-text' : 'text-slate-400'">
                  {{ isVip ? '已开通' : '未开通' }}
                </span>
                <span v-if="isVip"
                  class="text-xs bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded border border-emerald-100">
                  剩余 {{ vipDaysLeft }} 天
                </span>
              </div>
            </div>
            <button @click="navigate('#/plan')"
              class="theme-bg text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg text-sm font-medium shadow-sm shrink-0">
              {{ isVip ? '续费通用' : '开通通用' }}
            </button>
          </div>
        </div>
          ...
          <div class="px-5 sm:px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              ...
              <p class="text-[11px] text-slate-400 mt-0.5">
                套餐总价含最多 {{ publicSettings.custom_max_symbols || 3 }} 只 · 与通用独立 · 不解锁通用图表
              </p>
            </div>
            <button @click="openCustomEditor"
              class="text-xs theme-bg text-white px-3 py-1.5 rounded-lg self-start">
              <i class="fa-solid fa-plus mr-1"></i>添加标的
            </button>
          </div>
          <div v-if="customLoading" class="p-6 text-center text-slate-400 text-sm">
            <i class="fa-solid fa-spinner animate-spin"></i>
          </div>
          <div v-else-if="!customWatchlist.length" class="p-8 text-center text-sm text-slate-400">
            暂无定制标的
          </div>
          <div v-else class="overflow-x-auto">
            <table class="w-full text-sm whitespace-nowrap">
              <thead class="bg-slate-50 text-xs text-slate-500 border-b">
                <tr>
                  <th class="py-2.5 px-4 text-left">代码/名称</th>
                  <th class="py-2.5 px-3">状态</th>
                  <th class="py-2.5 px-3">到期</th>
              ...
              <tbody class="divide-y divide-slate-50">
                <tr v-for="item in customWatchlist" :key="item.id">
                  <td class="py-3 px-4 text-left">
                    <div class="font-mono font-bold">{{ item.etf_code }}</div>
                  ...
                  <td class="py-3 px-3">
                    <span class="text-xs px-2 py-0.5 rounded-full"
                      :class="item.status==='active'
                        ...
                        : (item.status==='pending' ? 'bg-orange-50 text-orange-600' : 'bg-slate-100 text-slate-400')">
                  ...
                  <td class="py-3 px-3 text-right">
                    <button @click="removeCustomItem(item)" class="text-xs text-slate-400 hover:text-red-500">
                      移除
                    </button>
                  </td>
                </tr>
              ...
              <tbody class="divide-y divide-slate-50">
                <tr v-for="order in orderList" :key="order.id" class="hover:bg-slate-50">
                  ...
                  <td class="py-3 px-4 text-xs">
                    {{ order.order_type==='custom_watchlist' ? '定制' : '通用' }}
                  </td>
                  <td class="py-3 px-4 text-emerald-600 font-medium text-xs">
                    {{ order.status==='approved'
                      ? (order.vip_days_granted
                          ? ('+'+order.vip_days_granted+'天')
                          : (order.order_type==='custom_watchlist' ? '定制激活' : '-'))
                      : '-' }}
                  </td>
                  <td class="py-3 px-4">
                    <span :class="order.status==='approved'
                      ? 'text-emerald-600'
                      : (order.status==='pending' ? 'text-orange-500' : 'text-slate-400')">
                ...
                <tr v-if="!orderList.length">
                  <td colspan="6" class="py-8 text-center text-slate-400 text-sm">暂无订单</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-slate-100 space-y-4">
          ...
          <div class="bg-slate-50 rounded-xl p-4 sm:p-5 border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <span class="font-mono text-2xl font-bold text-slate-700 tracking-widest">
                {{ referralCode || '-' }}
              </span>
            </div>
            <div class="sm:text-right text-sm theme-text font-medium leading-relaxed">
              ...
              <span class="text-base font-bold">
                邀请人 {{ publicSettings.gift_inviter_days || 3 }} 天 ·
                被邀请人 {{ publicSettings.gift_invitee_days || 2 }} 天
              </span>
            </div>
          </div>
          <div>
            <div class="text-sm font-medium text-slate-600 mb-2">
              我邀请的用户 <span class="text-xs text-slate-400">({{ inviteeList.length }})</span>
            </div>
              ...
              <table class="w-full text-sm">
                <thead class="text-xs text-slate-400 border-b">
                  <tr>
                    ...
                    <th class="py-2 text-left">当前VIP天数</th>
                ...
                <tbody class="divide-y divide-slate-50">
                  <tr v-for="inv in inviteeList" :key="inv.id">
                    ...
                    <td class="py-2.5" :class="inv.vip_days_left>0 ? 'text-emerald-500 font-bold' : ''">
                      {{ inv.vip_days_left }} 天
                    </td>
        ...
        <div class="bg-white p-5 sm:p-6 rounded-xl shadow-sm border border-slate-100">
          ...
          <div class="space-y-3 max-w-md">
            <input v-model="pwdForm.old" type="password" placeholder="旧密码"
              class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm">
            <input v-model="pwdForm.new" type="password" placeholder="新密码"
              class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm">
            <input v-model="pwdForm.confirm" type="password" placeholder="确认新密码"
              class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm">
            <button @click="submitPasswordChange" :disabled="pwdLoading"
              class="theme-bg text-white px-6 py-2 rounded-lg text-sm disabled:opacity-50">
              {{ pwdLoading ? '保存中...' : '保 存' }}
            </button>
          </div>
        </div>
