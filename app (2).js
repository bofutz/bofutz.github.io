...
createApp({
  setup() {
    ...
    /**
     * 免费 TopN：锁定「全站最新一期数据」的振幅前 N 只代码。
     * - 若存在最新日线 day_status：按该日 |日线| 排名（如周一收盘后）
     * - 否则按最新可用周线 |week_status| 排名（如周一未收盘、仅周线）
     * - 与用户所选历史周、列排序、搜索完全无关；新数据到达后自动重算
     */
    const computeLockedFreeTop = () => {
      ...
      rows.forEach((item) => {
        ...
        if (
          ...
          if (!latestDayDate || item.date > latestDayDate) latestDayDate = item.date;
        }
      });

      // 2) 最新有周线的日期（用于推周一）
      let latestWeekAnchor = null;
      rows.forEach((item) => {
        if (!inScope(item.etf_code)) return;
        if (
          item.week_status &&
          ...
          if (!latestWeekAnchor || item.date > latestWeekAnchor) latestWeekAnchor = item.date;
        }
      });

      const rankMap = {};

      // 判定：若「最新日线」属于「最新周」内，且该周已有日线，则用日线；
      // 若最新周尚无任意日线（例如新周一未收盘），则用上一完整周的周线排名。
      let useDay = false;
      if (latestDayDate) {
        if (!latestWeekAnchor) {
          useDay = true;
        } else {
          const latestWeekDays = getWeekDays(latestWeekAnchor);
          const dayInLatestWeek = latestWeekDays.includes(latestDayDate);
          // 最新周内是否已有任意日线
          const latestWeekHasDay = rows.some(
            (item) =>
              inScope(item.etf_code) &&
              latestWeekDays.includes(item.date) &&
              item.day_status &&
              item.day_status !== "-" &&
              item.day_status !== "--"
          );
          // 日线不在最新周 → 说明最新周还没日线，周线更新可能仍是旧周；用周线
          // 日线在最新周 → 用日线
          useDay = dayInLatestWeek && latestWeekHasDay;
        }
      }

      if (useDay && latestDayDate) {
        rows.forEach((item) => {
          if (item.date !== latestDayDate || !inScope(item.etf_code)) return;
          if (!item.day_status || item.day_status === "-" || item.day_status === "--") return;
          const val = Math.abs(getStatusVal(item.day_status));
          if (val === 9999 || val < 0) return;
          if (rankMap[item.etf_code] == null || val > rankMap[item.etf_code]) {
            rankMap[item.etf_code] = val;
          }
        });
      } else if (latestWeekAnchor) {
        // 用最新周线所在周的 week_status 排名（同一周多条取该 code 的 week_status）
        const weekDays = getWeekDays(latestWeekAnchor);
        rows.forEach((item) => {
          if (!inScope(item.etf_code)) return;
          if (!weekDays.includes(item.date)) return;
          if (!item.week_status || item.week_status === "-" || item.week_status === "--") return;
          const val = Math.abs(getStatusVal(item.week_status));
          if (rankMap[item.etf_code] == null || val > rankMap[item.etf_code]) {
            rankMap[item.etf_code] = val;
          }
        });
      ...
      const codes = Object.entries(rankMap)
        .map(([code, val]) => ({ code, val }))
        .sort((a, b) => b.val - a.val)
        .slice(0, n)
        .map((x) => x.code);

      freeEtfCodes.value = codes;
    };

    // 仅在源数据/共用列表/N 变化时重算；绝不依赖 selectedMonday / sortColumn / sortedData
    watch(
      [allData, sharedWatchlist, () => publicSettings.value.free_top_n_charts],
      () => {
